...
ScholarOS sends `SCHOLAROS_LOCKIN_REQUEST` with a unique `requestId`, current bounded dashboard snapshot, and reduced account context for sanitized status and seven-day analytics. The content script saves those current values before asking the background worker to synchronize with LockIn. This ordering keeps **Sync Poke now** from reporting success for a stale day page. The extension answers with `STUDYX_LOCKIN_RESULT`. The website never connects to LockIn directly and never receives the LockIn bearer token, raw domain lists, filesystem paths, or top-requested-domain details. The extension may connect over loopback or an authenticated HTTPS tunnel when Chrome restricts loopback access.
...
The extension also posts a sanitized companion snapshot to `/bridge/v1/companion/sync` every 30 seconds and receives Poke commands under a short delivery lease. Supported commands are `start_session`, `stop_session`, `resolve_access_request`, `set_tiktok_policy`, and `block_tiktok`. Before execution, the extension requires a valid command UUID, its own device ID, a pending/delivered state, an unexpired command deadline, and (for delivered commands) a live delivery lease. It durably reserves the command ID before mutation and stores the final receipt afterward. Lease-based redelivery therefore acknowledges the receipt instead of repeating a mutation; if Chrome terminated inside that narrow interval, the command reports an interruption and must be explicitly retried. Results are acknowledged through `/bridge/v1/companion/ack`. The snapshot includes aggregate TikTok foreground and scrolling seconds plus the active ScholarOS day mode, current-mode checklist, ignored contexts, and deterministic before-screen-time gate. Tokens, TikTok content, arbitrary tab data, browsing history, and captured page text are excluded.

The ScholarOS gate is valid only when its date matches the device's current local date. The extension recomputes it solely from incomplete current-mode checklist items explicitly marked `beforeScreenTime`; it does not trust the page's precomputed `shouldBlock`, counts, incomplete-item list, or reason, and Poke cannot add its own reasons. A mode change creates a new context ID and invalidates the previous gate. Manual and daily-limit TikTok blocks are evaluated separately.
